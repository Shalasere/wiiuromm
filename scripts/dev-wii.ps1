...
$appLogPath = Join-Path $logDir "wiiuromm-app.log"

$dolphin = Find-DolphinBinary
if (-not $dolphin) {
    throw "Dolphin not found. Run scripts/install-emulators-windows.ps1 first."
}
...
Set-Item -Path Env:ROMM_LOG_PATH -Value $appLogPath
...
Write-Host "[run] Launching Dolphin with $buildOut (user dir: $userDir)"
$bridgeProc = Start-RommLocalBridge -RepoRoot $repoRoot -ServerUrl $resolvedServerUrl
try {
    $dolphinProc = Start-Process -FilePath $dolphin -ArgumentList @("/u", $userDir, "/e", $buildOut) -PassThru
    Start-Sleep -Milliseconds 700
    try {
        Add-Type -AssemblyName Microsoft.VisualBasic -ErrorAction Stop
        [void][Microsoft.VisualBasic.Interaction]::AppActivate($dolphinProc.Id)
    } catch {
        # Foreground activation is best-effort only.
    }
    Wait-Process -Id $dolphinProc.Id
} finally {
