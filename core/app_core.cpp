...
std::vector<int> visibleRomIndices(const Status& status, const PlatformEntry& platform) {
    ...
    for (size_t i = 0; i < platform.roms.size(); ++i) {
        ...
        if (containsCaseInsensitive(rom.title, status.searchQuery) || containsCaseInsensitive(rom.subtitle, status.searchQuery) || containsCaseInsensitive(platform.name, status.searchQuery)) {
            indices.push_back(static_cast<int>(i));
        }
    }

    auto cmpTitle = [&platform](int a, int b) {
        if (platform.roms[static_cast<size_t>(a)].title == platform.roms[static_cast<size_t>(b)].title) {
            return a < b;
        }
        return platform.roms[static_cast<size_t>(a)].title < platform.roms[static_cast<size_t>(b)].title;
    };
...
bool existsInQueueOrCompletedHistory(const Status& status, const std::string& romId) {
    for (const auto& item : status.downloadQueue) {
        if (item.rom.id == romId)
            return true;
    }
    for (const auto& item : status.downloadHistory) {
        if (item.rom.id == romId && item.state == QueueState::Completed)
            return true;
    }
    return false;
}
...
bool removeSelectedQueueItem(Status& status) {
    ...
    if (status.activeDownloadIndex > idx)
        status.activeDownloadIndex--;
...
Status makeDefaultStatus() {
    ...
    status.lastMessage = "Waiting for live platform index...";
    return status;
}
...
ApplyResult applyAction(Status& status, Action action) {
    ...
    case Action::Select:
        if (status.currentView == Status::View::PLATFORMS) {
            if (status.platforms.empty()) {
                status.lastMessage = "Platforms still loading...";
            } else {
            ...
            if (status.updaterState == UpdaterState::Idle || status.updaterState == UpdaterState::Checking) {
    ...
    case Action::StartDownload:
        if (status.currentView == Status::View::QUEUE) {
            ...
            result.stateChanged = true;
        } else if (status.currentView == Status::View::DOWNLOADING && status.downloadWorkerRunning) {
...
bool tickDownload(Status& status, uint32_t elapsedMs) {
    ...
    if (status.activeDownloadIndex < 0 || status.activeDownloadIndex >= static_cast<int>(status.downloadQueue.size())) {
        return beginNextPendingDownload(status);
    }
...
const char* screenHint(Status::View view) {
    switch (view) {
    case Status::View::PLATFORMS:
        return "A: open platform, R/Z: diagnostics, L: updater";
    case Status::View::ROMS:
        return "A: detail, B: back, -/R: search, Left/Right: sort";
    case Status::View::DETAIL:
        return "A: add to queue, B: back, Y/1: queue";
    case Status::View::QUEUE:
        return "A: retry failed, Left: remove, Right: clear, X/2: start";
    case Status::View::DOWNLOADING:
        return "X/2: pause/resume, B: queue, HOME/+ or START: quit";
    case Status::View::DIAGNOSTICS:
        return "A: export summary, B: back";
    case Status::View::UPDATER:
        return "A: check/apply update, B: back";
    case Status::View::ERROR:
        return "B: return to platforms";
    default:
        return "";
    }
}
...
ViewPalette paletteForView(Status::View view) {
    switch (view) {
    case Status::View::PLATFORMS:
        return {{6, 46, 112}, {38, 108, 200}};
    case Status::View::ROMS:
        return {{0, 70, 96}, {20, 142, 186}};
    case Status::View::DETAIL:
        return {{12, 26, 72}, {54, 110, 210}};
    case Status::View::QUEUE:
        return {{52, 26, 88}, {120, 72, 180}};
    case Status::View::DOWNLOADING:
        return {{90, 60, 0}, {140, 100, 20}};
    case Status::View::DIAGNOSTICS:
        return {{30, 70, 40}, {40, 120, 70}};
    case Status::View::UPDATER:
        return {{16, 20, 70}, {50, 70, 170}};
    case Status::View::ERROR:
        return {{90, 0, 0}, {150, 20, 20}};
    default:
        return {{24, 24, 24}, {200, 200, 200}};
    }
}
...
std::vector<std::string> buildStatusLines(const Status& status, bool colorize) {
    ...
    if (platform != nullptr) {
        const std::vector<int> visible = visibleRomIndices(status, *platform);
        lines.emplace_back("Platform     : " + platform->name + " (" + std::to_string(status.selectedPlatformIndex + 1) + "/" + std::to_string(status.platforms.size()) + ")");
        lines.emplace_back("Visible ROMs : " + std::to_string(visible.size()) + "/" + std::to_string(platform->roms.size()));
        ...
        if (!visible.empty()) {
            ...
            lines.emplace_back("ROM          : " + rom.title + " [" + std::to_string(selectedVisibleIdx + 1) + "/" + std::to_string(visible.size()) + "]");
    ...
    lines.emplace_back("Queue        : " + std::to_string(status.downloadQueue.size()) + " items (" + std::to_string(pendingCount) + " pending, " + std::to_string(failedCount) + " failed)");
    ...
    if (status.downloadWorkerRunning && status.activeDownloadIndex >= 0 && status.activeDownloadIndex < static_cast<int>(status.downloadQueue.size())) {
        const QueueItem& item = status.downloadQueue[static_cast<size_t>(status.activeDownloadIndex)];
        lines.emplace_back("Downloading  : " + item.rom.title + " (" + std::to_string(item.progressPercent) + "%, try " + std::to_string(item.attempts) + ")");
    }
    ...
    lines.emplace_back("Updater      : " + std::string(updaterStateName(status.updaterState)) + " (current " + status.currentVersion + ", latest " + status.availableVersion + ")");
    lines.emplace_back("Diag         : input=" + std::to_string(status.diagnostics.inputCount) + " enqueue=" + std::to_string(status.diagnostics.enqueueCount) + " dup=" + std::to_string(status.diagnostics.duplicateBlockedCount) + " done=" + std::to_string(status.diagnostics.completedCount) + " fail=" + std::to_string(status.diagnostics.failedCount) + " search=" + std::to_string(status.diagnostics.searchCount));
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        ...
        if (startsWith(line, "Message      : ")) {
            ...
            } else if (line.find("complete") != std::string::npos || line.find("Updated") != std::string::npos) {
...
std::vector<std::string> buildFramedStatusLines(const Status& status, bool colorize) {
    ...
    out.emplace_back("======================================================================");
    out.emplace_back("wiiuromm | " + std::string(screenTitle(status.currentView)) + " | " + std::string(viewName(status.currentView)) + " | " + busyLabel);
    ...
    out.emplace_back("----------------------------------------------------------------------");
    out.emplace_back("| footer: queue=" + std::to_string(status.downloadQueue.size()) + " history=" + std::to_string(status.downloadHistory.size()) + " msg=\"" + stripAnsi(status.lastMessage) + "\"");
    out.emplace_back("======================================================================");
    return out;
}
